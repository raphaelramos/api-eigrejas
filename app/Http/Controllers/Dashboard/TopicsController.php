...
use App\Models\{AttachFile, Comment, Map, Photo, RelatedTopic, Section, 
    Topic, TopicCategory, TopicField, WebmasterSection};
use Auth; use File; use Helper; use Redirect; use Form; 
use URL; use Image; use Cache;
...
class TopicsController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        if ($totalFiltered > 0) {
            ...
            foreach ($Topics as $Topic) {
                ...
                $photo = "";
                if ($Topic->photo_file != "") {
                    $photo = " <div class=\"pull-right\"><img src=\"" . asset($Topic->photo_file) . "\" style=\"height: 40px\" alt=\"" . $title . "\"></div>";
                }
    ...
    // Add rel="nofollow" in external links
    public function nofollow($html) {
        $dom = new \DOMDocument();
    ...
    public function store(Request $request, $webmasterId)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            $this->validate($request, [
                'photo_file' => ['mimes:png,jpeg,jpg,gif,webp','max:15240'],
                'audio_file' => ['mimes:mpga,wav,mp3','max:15240'], // mpga = mp3
                'video_file' => ['mimes:mp4,ogv,webm','max:15240'] // max 15mb
            ]);
            ...
            $audioFileFinalName = "";
            if ($request->hasFile($formFileName)) {
                $audioFileFinalName = uniqid() . '.' . $request->file($formFileName)->getClientOriginalExtension();
            ...
            $attachFileFinalName = "";
            if ($request->hasFile($formFileName)) {
                $attachFileFinalName = uniqid() . '.' . $request->file($formFileName)->getClientOriginalExtension();
            ...
            if ($request->video_type == 3) {
            ...
            } elseif ($request->youtube_channel) {
                $videoFileFinalName = Helper::Get_youtube_channel_ID($request->youtube_channel);
                if ($videoFileFinalName == '') {
                    return redirect()->action('Dashboard\TopicsController@create', [$webmasterId])
                        ->withErrors(__('validation.channel'))
                        ->withInput();
                }
            } else {
                ...
                $videoFileFinalName = "";
                if ($request->hasFile($formFileName)) {
                    $videoFileFinalName = uniqid() . '.' . $request->file($formFileName)->getClientOriginalExtension();
                    $request->file($formFileName)->move($path, $videoFileFinalName);
                    $videoFileFinalName = $path.$videoFileFinalName;
                }
            ...
            $Topic->date = Helper::dateForDB($request->date, 1);
            if (@$request->expire_date != "") {
                $Topic->expire_date = Helper::dateForDB($request->expire_date);
            }
            if ($fileFinalName != "") {
                $Topic->photo_file = $path.$fileFinalName;
            }
            if ($audioFileFinalName != "") {
                $Topic->audio_file = $path.$audioFileFinalName;
            }
            if ($attachFileFinalName != "") {
                $Topic->attach_file = $path.$attachFileFinalName;
            }
            if ($videoFileFinalName != "") {
                    ...
                    if (in_array(Auth::user()->permissions_id, $add_permission_groups) || in_array(0, $add_permission_groups) || $customField->add_permission_groups == "") {
                        ...
                        if ($request->$field_value_var != "") {
                            if ($customField->type == 8 || $customField->type == 9 || $customField->type == 10) {
                                // upload file
                                if ($request->$field_value_var != "") {
                                    $uploadedFileFinalName = uniqid() . '.' . $request->file($field_value_var)->getClientOriginalExtension();
                                    $path = @Helper::uploadPath('topics');
            ...
            return redirect()->action('Dashboard\TopicsController@edit', [$webmasterId, $Topic->id])->with(['doneMessage' =>
                __('backend.addDone'), 'webmasterId' => $webmasterId]);
        } else {
            return redirect()->route('NotFound');
        }
    }
            ...
            if (!empty($Topic)) {
                ...
                if ($request->hasFile($formFileName)) {
                    ...
                    if ($Topic->$formFileName != "") {
                        File::delete($Topic->$formFileName);
                    }
                ...
                if ($request->video_type == 3) {
                    ...
                    $videoFileFinalName = Helper::Get_youtube_channel_ID($request->youtube_channel);
                    if ($videoFileFinalName == '') {
                        return redirect()->action('Dashboard\TopicsController@edit', [$webmasterId, $id])
                            ->withErrors(__('validation.channel'))
                            ->withInput();
                    }
                } else {
                        ...
                        if (in_array(Auth::user()->permissions_id, $edit_permission_groups) || in_array(0, $edit_permission_groups) || $customField->edit_permission_groups == "") {
                            ...
                            if ($customField->type == 8 || $customField->type == 9 || $customField->type == 10) {
                                ...
                                if ($request->$file_del_id) {
                                    ...
                                    File::delete($request->$file_old_id);
                                    $field_value = "";
                                }
                            } elseif ($customField->type == 5) {
                                if ($request->$field_value_var != "") {
    ...
    public function destroy($webmasterId, $id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            if (!empty($Topic)) {
                ...
                if ($Topic->audio_file != "") {
                    File::delete($Topic->audio_file);
                }
                if ($Topic->video_type == 0 && $Topic->video_file != "") {
                    File::delete($Topic->video_file);
                }
                    ...
                    if ($request->action == "activate") {
                        ...
                        foreach ($AttachFile_Files as $AttachFile_File) {
                            if ($AttachFile_File->file != "") {
                                File::delete($AttachFile_File->file);
                            }
                        }
    ...
    public function photos(Request $request, $webmasterId, $id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            $formFileName = "file";
            $fileFinalName = "";
            $fileFinalTitle = ""; // Original file name
            if ($request->hasFile($formFileName)) {
...
// Files Functions
    ...
    public function filesStore(Request $request, $webmasterId, $id)
    {
        ...
        if (!empty($WebmasterSection)) {
            ...
            $fileFinalName = "";
            if ($request->hasFile($formFileName)) {
                $fileFinalName = uniqid() . '.' . $request->file($formFileName)->getClientOriginalExtension();
            ...
            if (!empty($AttachFile)) {
                ...
                if ($request->hasFile($formFileName)) {
                    ...
                    if ($AttachFile->$formFileName != "") {
                        File::delete($AttachFile->$formFileName);
                    }
