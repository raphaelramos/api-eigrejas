...
use App;
use App\Models\{AnalyticsPage as AnalyticsPage, AnalyticsVisitor as AnalyticsVisitor, Banner as Banner, 
	Country as Country, Event as Event, Menu as Menu,
    Section as Section, Setting as Setting, Topic as Topic, Webmail as Webmail, Language as Language,
    WebmasterSection as WebmasterSection, WebmasterSetting as WebmasterSetting};
use Illuminate\Support\Carbon;
use Illuminate\Support\Str;
use Auth; use GeoIP; use Config; use Http;
...
class Helper
{
    ...
    {
        return 'https://admin.eigrejas.com/church/set/'.tenant('id');
    }

    static function adminPanel()
    {
        return 'https://admin.eigrejas.com/panel';
    }
    ...
    static function uploadPath($folder)
    {
        $path = "uploads/tenants/" . tenant('id') . "/" . $folder . "/" . date('ym') . "/";
    ...
    // Visitors Data
    static function SaveVisitorInfo($PageTitle, $local = "web")
    {
    ...
    static function Get_youtube_channel_ID($url){
    ...
    static function GetIcon($file)
    {
        ...
        if ($ext == ".pdf") {
            $icon = "<i class=\"fas fa-file-pdf\" style='color: red;font-size: 20px'></i>";
        }
        if ($ext == '.png' or $ext == '.jpg' or $ext == '.jpeg' or $ext == '.gif') {
            $icon = "<img src='$file' style='width: auto;height: 20px' title=''>";
        }
        if ($ext == ".xls" or $ext == '.xlsx') {
            $icon = "<i class=\"fas fa-file-excel-o\" style='color: green;font-size: 20px'></i>";
        }
        if ($ext == ".ppt" or $ext == '.pptx' or $ext == '.pptm') {
            $icon = "<i class=\"fas fa-file-powerpoint\" style='color: #1066E7;font-size: 20px'></i>";
        }
        if ($ext == ".doc" or $ext == '.docx') {
            $icon = "<i class=\"fas fa-file-word\" style='color: #0EA8DD;font-size: 20px'></i>";
        }
        if ($ext == ".zip" or $ext == '.rar') {
            $icon = "<i class=\"fas fa-file-zip\" style='color: #C8841D;font-size: 20px'></i>";
        }
        if ($ext == ".txt" or $ext == '.rtf') {
            $icon = "<i class=\"fas fa-file-text\" style='color: #7573AA;font-size: 20px'></i>";
        }
        if ($ext == ".mp3" or $ext == '.wav') {
            $icon = "<i class=\"fas fa-file-audio\" style='color: #8EA657;font-size: 20px'></i>";
        }
        if ($ext == ".mp4" or $ext == '.avi') {
            $icon = "<i class=\"fas fa-file-video\" style='color: #D30789;font-size: 20px'></i>";
        }
        return $icon;
    ...
    static function currentLanguage()
    {
        ...
        if (empty($Language)) {
            $Language = Language::where("code", Config::get('app.locale'))->first();
        }
        return $Language;
    }
        ...
        try {
            ...
            if (!empty($WebmasterSection)) {
                ...
                if ($WebmasterSection->{'seo_url_slug_' . $lang} != "") {
                    ...
                    $slug = $WebmasterSection->{'seo_url_slug_' . Config::get('app.locale')};
                }
                if ($slug == "") {
                    $slug = Str::slug($title, '-');
                }
    ...
    static function categoryURL($id, $lang = "", $path = "")
    {
        $category_url = "";
        try {
            $title_var = "title_" . @Helper::currentLanguage()->code;
            ...
            if (!empty($Category)) {
                ...
                if ($Category->{'seo_url_slug_' . $lang} != "") {
                    ...
                    $slug = $Category->{'seo_url_slug_' . Config::get('app.locale')};
                }
                if ($slug == "") {
                    $slug = Str::slug($title, '-');
                }
                ...
                if (!empty($WebmasterSection)) {
                    ...
                    if ($WebmasterSection->{'seo_url_slug_' . $lang} != "") {
                        ...
                        $WebmasterSection_slug = $WebmasterSection->{'seo_url_slug_' . Config::get('app.locale')};
                    }
    ...
    static function topicURL($id, $lang = "", $path = "")
    {
        $topic_url = "";
        try {
            $title_var = "title_" . @Helper::currentLanguage()->code;
            ...
            if (!empty($Topic)) {
                ...
                if ($Topic->{'seo_url_slug_' . $lang} != "") {
                    ...
                    $slug = $Topic->{'seo_url_slug_' . Config::get('app.locale')};
                }
                if ($slug == "") {
                    $slug = Str::slug($title, '-');
                }
    ...
    {
        $format = Config::get('front.date_format');
        if ($date != "") {
    ...
    static function formatDateTime($date = "")
    {
        ...
        $timezone = $WebmasterSettings->timezone ?? config('app.timezone');

        if ($date != "") {
            return Carbon::parse($date)->timezone($timezone)->format('Y-m-d\TH:i');
        }
        return Carbon::now()->timezone($timezone)->format('Y-m-d\TH:i');
    }

    static function formatDateC($date = "")
    {
        if ($date != "") {
    ...
    static function timeForDB($date = "")
    {
        if ($date) {
            try {
    ...
    static function onlyNumber($number)
    {
