...
class WebmasterSettingsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $settings->newsletter_status = false;
        $settings->members_status = false;
        $settings->orders_status = false;
        $settings->shop_status = false;
        $settings->shop_settings_status = false;
        $settings->default_currency_id = "3";
        $settings->languages_by_default = "pt";

        $settings->header_menu_id = "1";
        $settings->footer_menu_id = "2";
        $settings->app_menu_id = "1";
        $settings->social_menu_id = "3";
        $settings->app_banners_section_id = "1";
        $settings->app_sections = "2, 3, 4, 5, 6";
        $settings->home_banners_section_id = "1";
        $settings->home_text_banners_section_id = "2";
        $settings->side_banners_section_id = "3";
        $settings->stories_banners_section_id = "4";
        $settings->contact_page_id = "2";
        $settings->newsletter_contacts_group = "1";
        $settings->new_comments_status = true;
        $settings->home_content1_section_id = "3";
        $settings->home_content2_section_id = "2";
        $settings->home_content3_section_id = "";
        $settings->home_content_photo_section_id = "6";
        $settings->home_content_channel_section_id = "4";
        $settings->home_contents_per_page = "6";
        $settings->latest_news_section_id = "";
        $settings->links_status = true;
        $settings->register_status = false;
        $settings->member_register_status = false;
        $settings->permission_group = "3";
        $settings->api_status = true;
        $settings->api_key = uniqid();
        $settings->group_name = "Célula";
        $settings->groups_name = "Células";

        $settings->mail_driver = "sendmail";
        $settings->mail_host = "";
        $settings->mail_port = "";
        $settings->mail_username = "";
        $settings->mail_password = "";
        $settings->mail_encryption = "";
        $settings->mail_no_replay = "naoresponda@eigrejas.com";
