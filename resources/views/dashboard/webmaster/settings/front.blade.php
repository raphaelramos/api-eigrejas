<div
    ...
    <div class="p-a-md col-lg-12">
        <div class="col-lg-6">
            ...
            <div class="form-group">
                <label>{{ __('backend.socialMenu') }} : </label>
                <select name="social_menu_id" id="social_menu_id" class="form-control c-select">
                    <option value="0">- - {!!  __('backend.none') !!} - -</option>
                    <?php
                    $title_var = "title_" . @Helper::currentLanguage()->code;
                    $title_var2 = "title_" . \Config::get('app.locale');
                    ...
                    @foreach ($ParentMenus as $ParentMenu)
                        ...
                        ?>
                        <option
                            value="{{ $ParentMenu->id  }}" {{ ($ParentMenu->id == $WebmasterSetting->social_menu_id) ? "selected='selected'":""  }}>{{ $title }}</option>
                    @endforeach
                </select>
            </div>
        ...
        <div class="col-lg-6">
                ...
                <select name="home_content1_section_id" id="home_content1_section_id"
                    ...
                    @foreach ($GeneralWebmasterSections as $Webmaster_Section)
                        @if($Webmaster_Section->type !=4)
                            ...
                            ?>
                            <option value="{{ $Webmaster_Section->id  }}" {{ ($Webmaster_Section->id == $WebmasterSetting->home_content1_section_id) ? "selected='selected'":""  }}>{!! $WSectionTitle !!}</option>
                        @endif
                    @endforeach
                </select>
            </div>
                ...
                <select name="home_content2_section_id" id="home_content2_section_id"
                    ...
                    @foreach ($GeneralWebmasterSections as $Webmaster_Section)
                        @if($Webmaster_Section->type !=4)
                            ...
                            ?>
                            <option value="{{ $Webmaster_Section->id  }}" {{ ($Webmaster_Section->id == $WebmasterSetting->home_content2_section_id) ? "selected='selected'":""  }}>{!! $WSectionTitle !!}</option>
                        @endif
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                <label>{{ __('backend.homeRowPhoto') }} : </label>
                <select name="home_content_photo_section_id" id="home_content_photo_section_id" class="form-control c-select">
                    <option value="0">- - {!!  __('backend.none') !!} - -</option>
                    @foreach ($GeneralWebmasterSections as $Webmaster_Section)
                        @if($Webmaster_Section->type ==1)
                            <?php
                            if ($Webmaster_Section->$title_var != "") {
                                $WSectionTitle = $Webmaster_Section->$title_var;
                            } else {
                            ...
                            ?>
                            <option value="{{ $Webmaster_Section->id  }}" {{ ($Webmaster_Section->id == $WebmasterSetting->home_content_photo_section_id) ? "selected='selected'":""  }}>{!! $WSectionTitle !!}</option>
                        @endif
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                <label>{{ __('backend.homeRowChannel') }} : </label>
                <select name="home_content_channel_section_id" id="home_content_channel_section_id" class="form-control c-select">
                    <option value="0">- - {!!  __('backend.none') !!} - -</option>
                    @foreach ($GeneralWebmasterSections as $Webmaster_Section)
                        @if($Webmaster_Section->type ==6)
                            <?php
                            if ($Webmaster_Section->$title_var != "") {
                                $WSectionTitle = $Webmaster_Section->$title_var;
                            } else {
                            ...
                            ?>
                            <option value="{{ $Webmaster_Section->id  }}" {{ ($Webmaster_Section->id == $WebmasterSetting->home_content_channel_section_id) ? "selected='selected'":""  }}>{!! $WSectionTitle !!}</option>
                        @endif
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                <label>{{ __('backend.homeRow5') }} : </label>
                <select name="home_content3_section_id"
                        class="form-control c-select">
                    <option value="0">- - {!!  __('backend.none') !!} - -</option>
                    @foreach ($SitePages as $SitePage)
                        <?php
                        ...
                        ?>
                        <option
                            value="{{ $SitePage->id  }}" {{ ($SitePage->id == $WebmasterSetting->home_content3_section_id) ? "selected='selected'":""  }}>{{ $title }}</option>
                    @endforeach
                </select>
            </div>
                ...
                <select name="latest_news_section_id" id="latest_news_section_id"
                    ...
                    @foreach ($GeneralWebmasterSections as $Webmaster_Section)
                        @if($Webmaster_Section->type !=4)
                            ...
                            ?>
                            <option value="{{ $Webmaster_Section->id  }}" {{ ($Webmaster_Section->id == $WebmasterSetting->latest_news_section_id) ? "selected='selected'":""  }}>{!! $WSectionTitle !!}</option>
                        @endif
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group m-t-3">
                <label><h6>{{ __('backend.allowMemberRegister') }}</h6></label>
                <div class="radio">
                    <div style="margin-top: 5px;">
                        <label class="ui-check ui-check-md">
                            {!! Form::radio('member_register_status','1',$WebmasterSetting->member_register_status ? true : false , array('id' => 'member_register_status1','class'=>'has-value')) !!}
                            <i class="dark-white"></i>
                            {{ __('backend.active') }}
                        </label>
                    </div>
                    <div>
                        <label class="ui-check ui-check-md">
                            {!! Form::radio('member_register_status','0',$WebmasterSetting->member_register_status ? false : true , array('id' => 'member_register_status2','class'=>'has-value')) !!}
                            <i class="dark-white"></i>
                            {{ __('backend.notActive') }}
                        </label>
                    </div>
